...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        for (int i = 0; i < eligibleColumns.size(); i++) {
            SelectPredicate curr = this.selectPredicates.get(i);
            QueryOperator currIndexOp = new IndexScanOperator(this.transaction, curr.tableName, curr.column, curr.operator, curr.value);
            int currCost = currIndexOp.estimateIOCost();
            if (currCost < minCost) {
                minCost = currCost;
                minOp = currIndexOp;
     ...
     *
     * @param prevMap  maps a set of tables to a query operator (join) over the set of
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> set : prevMap.keySet()) {
            for (JoinPredicate joinPre : this.joinPredicates) {
                String leftTable = joinPre.leftTable;
                String rightTable = joinPre.rightTable;
                String leftColumn = joinPre.leftColumn;
                String rightColumn = joinPre.rightColumn;
                QueryOperator minCost;
                String lostTable;
                if (set.contains(leftTable) && !set.contains(rightTable)) {
                    lostTable = rightTable;
                    Set<String> oneSet = new HashSet<>();
                    oneSet.add(rightTable);
                    QueryOperator tableAccess = pass1Map.get(oneSet);
                    minCost = minCostJoinType(prevMap.get(set), tableAccess, leftColumn, rightColumn);
                } else if (set.contains(rightTable) && !set.contains(leftTable)) {
                    lostTable = leftTable;
                    Set<String> oneSet = new HashSet<>();
                    oneSet.add(leftTable);
                    QueryOperator tableAccess = pass1Map.get(oneSet);
                    minCost = minCostJoinType(tableAccess, prevMap.get(set), leftColumn, rightColumn);
                } else continue;
                Set<String> resSet = new HashSet<>();
                resSet.addAll(set);
                resSet.add(lostTable);
                result.put(resSet, minCost);
            }
        }
        // TODO(proj3_part2): implement
    ...
    public Iterator<Record> execute() {
        this.transaction.setAliasMap(this.aliases);
        // For each pass, we are joining together a set of i-1 tables that’s already joined,
        // (pass i - 1) with a new table (pass 1) that hasn’t been used.
        // We have to do this because we can only join 2 tables at a time,
        // and joining n - 1 times will lead to all of them being joined together.
        // In this example, all three tables should be joined to return the query.
        // In the example given, we need to join 3 - 1 = 2 times. Lmk what is still confusing you!


        // total pass number N
        int passTotal = this.tableNames.size();
        // pass1
        Map<Set<String>, QueryOperator> pass1Map = new HashMap<>();
        for (String table: this.tableNames) {
            QueryOperator minCostSingleAccess = minCostSingleAccess(table);
            Set<String> oneSet = new HashSet<>();
            oneSet.add(table);
            pass1Map.put(oneSet, minCostSingleAccess);
        }
        // pass 2 - i
        Map<Set<String>, QueryOperator> prevMap = pass1Map;
        // Map<Set<String>, QueryOperator> prevMap = new HashMap<>();
        // prevMap.putAll(pass1Map);
        for (int i = 1; i < passTotal; i++) {
