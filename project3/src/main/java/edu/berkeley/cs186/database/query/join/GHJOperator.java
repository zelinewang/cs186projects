...
public class GHJOperator extends JoinOperator {
        ...
        for (Record probeRecord: probeRecords) {
            ...
            for (Record bRecord : hashTable.get(probeJoinValue)) {
                ...
                if (!probeFirst) {
                    ...
                    joinedRecord = probeRecord.concat(bRecord); // always left.concat(right)
                }
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        for (int i = 0; i < leftPartitions.length; i++) {
            for (int j = 0; j < leftPartitions.length; j++) {
                if ((leftPartitions[i].getNumPages() <= numBuffers - 2)
                        || (rightPartitions[j].getNumPages() <= numBuffers - 2)) {
                    buildAndProbe(leftPartitions[i], rightPartitions[j]);
                } else {
                    run(leftPartitions[i], rightPartitions[j], pass + 1);
                    // For recursive call, assume the partition 0 is still too big.
                    // When I want to do the recursive call, I'm planning to call the run method again,
                    // I want to use the left and right records based on the current partition, which is 0 here.
                    //
                    // So we need to run() on records only in the 'too large' partitions
                    // to let them go in smaller partition to solve the joint,
                    // but not doing partitions on all the records all over again
                }
            }
    ...
    public static Pair<List<Record>, List<Record>> getBreakSHJInputs() {
        ...
        // As a reminder, SHJ does the following (assume we hash leftRecords and probe them against rightRecords):
        //
        //   Hash leftRecords into b-1 groups (b-1 since we need 1 page of memory to stream the input and therefore
        // we can use the remaining b-1 to stream the output).
        //   Load each of the b-1 groups into memory individually, build an in-memory hashtable out of the loaded group
        // and compare it against rightRecords. Since we need 1 buffer to stream rightRecords and 1 buffer to stream
        // the output, the hash-table built out of our loaded group (selected out of the b-1 groups) can be at most b-2 pages in size.
        // If the loaded group is more than b-2 pages in size, SHJ will not work.
        //
        // Therefore leftRecords can be at most (b - 1) * (b - 2) pages in size.
        // However, this assumes a uniform hash function so the actual maximum number of records supported by SHJ
    ...
    public static Pair<List<Record>, List<Record>> getBreakGHJInputs() {
        ...
        // breaking ghj "all tuples for a given key in same partition –
        // matching tuples between R and S get put in the same partition!"

        // populating both lists with records of the same value should be enough.
        return new Pair<>(leftRecords, rightRecords);
    }
}
