...
public class SortOperator extends QueryOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        for (int i = 0; i < runs.size(); i++) {
            ...
            iterators.add(thisRun.iterator());
            if (iterators.get(i).hasNext()) {
                Record record = iterators.get(i).next();
                priorityQueue.add(new Pair<>(record, i));
            }
        }

        Run mergeSortedRuns = new Run(transaction, getSchema());
        while (true) {
            Pair<Record, Integer> thisPair = priorityQueue.poll();
            if (thisPair == null) return mergeSortedRuns;
            else {
                mergeSortedRuns.add(thisPair.getFirst());
                if (iterators.get(thisPair.getSecond()).hasNext()) {
                    Record newRecord = iterators.get(thisPair.getSecond()).next();
                    priorityQueue.add(new Pair<>(newRecord, thisPair.getSecond()));
                }
            }
        }
        // TODO(proj3_part1): implement
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        List<Run> resSortedRuns = new ArrayList<>();
        for (int i = 0; i < (runs.size()/mergeNum); i++) {
            List<Run> unSorted = new ArrayList<>();
            for (int j = (i*mergeNum); j < (i + 1)*mergeNum; j++) {
                unSorted.add(runs.get(j));
            }
            resSortedRuns.add(i, mergeSortedRuns(unSorted));
        }

        List<Run> leftUnSorted = new ArrayList<>();
        for (int j = (runs.size()/mergeNum)*mergeNum; j < runs.size(); j++) {
            leftUnSorted.add(runs.get(j));
        }
        if (leftUnSorted.size() != 0) {
            resSortedRuns.add((runs.size()/mergeNum), mergeSortedRuns(leftUnSorted));
        }
        // TODO(proj3_part1): implement
        return resSortedRuns;
    }
    ...
    public Run sort() {
        ...
        } // initial the list of all sorted runs, a list of sorted runs
        while (sortedRuns.size() != 1) {
            List<Run> res = mergePass(sortedRuns);
            sortedRuns = res;
        }
        // TODO(proj3_part1): implement
