...
public class Database implements AutoCloseable {
    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                // TODO(proj4_part2)
                // At this point, transactions should be acquiring lots of locks needed to do their queries,
                // but no locks are ever released! We will be using Strict Two-Phase Locking in our database,
                // which means that lock releases only happen when the transaction finishes, in the cleanup method.
                // You should only use LockContext#release and not LockManager#release -
                // LockManger doesn't care about the restriction of Multi..
                List<Lock> lockList = lockManager.getLocks(this);
                for (int i = lockList.size()-1; i >= 0; i--) {
                    Lock thisLock = lockList.get(i);
                    lockList.remove(i);
                    if (!lockList.contains(thisLock)) {
                        LockContext cTxt = LockContext.fromResourceName(lockManager,thisLock.name);
                        cTxt.release(this);
                    }
                }
                return;
